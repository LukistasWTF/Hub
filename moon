--!strict
-- AutoMinigames Plus v2 – participación máxima
-- Juega agresivo, adapta cadencia, se reposiciona a objetivos, reclama recompensas,
-- gestiona colas correctamente y Anti‑AFK opcional. Debug con métricas. 💖
...
local Promise = (function() local ok,m=pcall(load,"package:Promise"); return ok and m or nil end)()
local LiveOpsTime = (function() local ok,m=pcall(load,"LiveOpsTime"); return ok and m or { now=function()return os.clock() end } end)()
local UIManager = (function() local ok,m=pcall(load,"UIManager"); return ok and m or nil end)()
local ClientToolManager = (function() local ok,m=pcall(load,"ClientToolManager"); return ok and m or { now=function()return LiveOpsTime.now() end } end)()
local MinigameClientManager = (function() local ok,m=pcall(load,"MinigameClientManager"); return ok and m or nil end)()
...
local CONFIG = {
	...
	-- Cadencia base (se auto‑ajusta dinámicamente)
	actionCadence = { joetation = 1.25, coconut_bonk = 1.5 },
	minCadence = 0.6,  -- límites de autoajuste (segundos / acción)
	maxCadence = 2.5,
	...
	moveAggressive = true,
	moveRadiusStop = 10, -- distancia para considerar que ya estamos en destino
...
local function ensureDebugGui()
	...
	title.TextSize = 18
	title.Text = "🤖 AutoMinigames Plus v2"
...
local function getMGLength(cli: MGClient, id: string): number
	...
	pcall(function() if (cli :: any).minigame_length then len = (cli :: any).minigame_length end end)
	return len
end
...
local dynCadence: {[string]: number} = table.clone(CONFIG.actionCadence)
...
local function updInfoLine()
	...
	for _, id in ipairs(CONFIG.order) do
		...
		table.insert(parts, string.format("%s: APM %d | cad %.2fs %s", id, apm, dynCadence[id] or 0, playing[id] and "🎮" or "⏸️"))
	end
...
-- Ajuste dinámico de cadencia según si hubo “actividad” tras la acción
...
local function leaveQueue(id: string, cli: MGClient?)
	...
	for _, r in ipairs(leaveRoutes) do
		local ok = pcall(function() cli:message_server(r) end)
		if ok then LOG(id, "🚪", "Saliendo de cola ("..r..")."); return true end
	end
	return false
end
local function ensureQueued(id: string, force: boolean?)
	...
	lastQueueTry[id] = t
	local cli = getClient(id); if not cli then LOG(id,"❌","Cliente no encontrado; reintento."); return end
	if cli.is_participating or cli.is_in_interior then return end
	local okJoin = pcall(function() cli:join_persistent_game() end)
	if (not okJoin) and cli.message_server then
		okJoin = pcall(function() cli:message_server("join_queue") end)
	end
	if okJoin then LOG(id, "🕒", "Intentando entrar a cola…") else LOG(id,"⚠️","Fallo al entrar a cola.") end
end
...
----------------------------
-- JOETATION: recoger bala + usar cañón cercano. Ajuste de cadencia por feedback.
local function act_joetation(cli: MGClient)
	...
	-- Ir hacia pila si la expone
	if cli.pickup_pile_position then moveTo(cli.pickup_pile_position) end
	-- Recoger y disparar
	local ok1 = pcall(function() cli:message_server("pickup_holdable_from_pile", pos, ClientToolManager.now()) end)
	-- Elegir cañón cercano si lo expone
	...
	local ok2 = pcall(function() cli:message_server("use_cannon", cannonIndex, pos, ClientToolManager.now()) end)
	actionsCount["joetation"] = (actionsCount["joetation"] or 0) + 1
	LOG("joetation","🎯", string.format("Disparo con cañón #%d", cannonIndex))
	-- Si hubo “actividad” (lastActivity se actualiza desde otros callbacks), asumimos positivo si no pasa mucho tiempo.
...
-- COCONUT BONK: recoger droppable y lanzar al barco vivo más cercano (o más grande)
local function act_coconut(cli: MGClient)
	...
	-- Targeting: vivo más cercano; si hay health, prioriza mayor vida.
	local me = (hrp() and hrp()!.Position) or Vector3.zero
	local bestUid, bestScore = nil, -1
	if cli.ships_by_uid then
		for uid, ship in cli.ships_by_uid do
			local alive, hp, p = true, 1, nil
			pcall(function()
	...
	if bestUid then
		local ok = pcall(function() cli:message_server("used_droppable", bestUid) end)
...
local function tryClaimRewards(id: string, cli: MGClient)
	for _, route in ipairs(rewardRoutes) do
		local ok = pcall(function() cli:message_server(route) end)
		if ok then LOG(id,"🏆","Recompensas reclamadas ("..route..")."); return end
	end
	if cli.ingame_app_controller then
		for _, m in ipairs({"claimRewards","ClaimRewards","Finish","finish","Complete","complete"}) do
			local f = (cli.ingame_app_controller :: any)[m]
			if typeof(f) == "function" then
				local ok = pcall(function() f(cli.ingame_app_controller) end)
				if ok then LOG(id,"🏆","Recompensas via controller ("..m..")."); return end
			end
		end
	end
end
...
local function onGameEnded(id: string, cli: MGClient)
	...
	mgEndsAt[id] = nil
	LOG(id,"🏁","Partida finalizada. Reclamando recompensas…")
	tryClaimRewards(id, cli)
	LOG(id,"🔁","Entrando en TODAS las colas.")
	joinAllQueues(true)
end
...
LOG("init","✨","AutoMinigames Plus v2 iniciado.")
LOG("init","🔁","Rotación: "..table.concat(CONFIG.order," → "))
LOG("init","🏆","Modo participación máxima activo.")
	...
	while true do
		for _, id in ipairs(CONFIG.order) do
			...
			if last and (LiveOpsTime.now() - last) > CONFIG.stuckTimeout then
				LOG(id,"🔄","Watchdog: sin actividad, re‑queue.")
				lastActivity[id] = LiveOpsTime.now()
			...
			if cli and cli.is_participating then
				...
				elseif id == "coconut_bonk" then
					...
					LOG(id,"🎮","Ping genérico.")
				end
				markActivity(id)
			end

			-- Countdown estimado (si no hay señal de fin)
...
-- Re‑cablear señales periódicamente
task.spawn(function()
	while true do
		for _, id in ipairs(CONFIG.order) do local cli = getClient(id); if cli then wireSignals(id, cli) end end
		task.wait(10)
...
pcall(function()
	LocalPlayer.Chatted:Connect(function(msg)
		if msg == "!automg panel" and debugGui then
			...
			LOG("ui","🖥️","Panel "..(debugGui.Enabled and "activado" or "oculto")..".")
		...
		elseif msg:match("^!automg move ") then
			...
			LOG("ui","🧭","Reposicionamiento "..(on and "ON" or "OFF"))
		end
	end)
end)
