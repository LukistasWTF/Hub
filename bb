-- AutoMinigames – Smart ETA + TP + FULL AutoPlay (v7_2)
-- Coloca este archivo como LocalScript en StarterPlayerScripts (cliente).

----------------------------------------------------------------------
-- Nota: este archivo NO usa pcall en la línea 1 para evitar
-- "attempt to call a nil value" si pcall está ensombrecido.
----------------------------------------------------------------------

------------------------------ Utilidades base ------------------------------
local SAFE_PCALL
do
    -- Crea un alias seguro de pcall: lo usa si existe; si no, ejecuta la función sin protección.
...
if not RunService:IsClient() then
    warn("[AutoMG] ❌ Debe ejecutarse como LocalScript (cliente).")
    return
end
...
local function loadM(name)
    ...
    -- 2) Búsqueda en ClientModules/SharedModules
...
local function logE(...) LOG("🛑 [AutoMG]", ...) end
local function logS(...) LOG("✅ [AutoMG]", ...) end

logI("Arrancando v7.2… (sin pcall en línea 1)")
...
local function pickBest()
    ...
    for _,id in ipairs(ORDER) do
        ...
        if r then
            logI(string.format("📡 ETA %-13s → ⏱ %s  (%s)  active=%s  inQueue=%s",
            ...
            logW(string.format("📡 ETA %-13s → sin datos (cliente no disponible)", id))
        end
    end
...
local function tuneCad(id, ok)
    ...
    dynCad[id] = cad
    logI(string.format("⏲️ Cadencia %-13s → %.2fs (ok=%s)", id, cad, tostring(ok)))
end
...
local function rpc(cli, route, ...)
    ...
    if not ok and CONFIG.verboseRPC then logW("📨 RPC FAIL:", route, err) end
    return ok
end

-- Cursores round‑robin
...
-- Joetation: recoger siempre y disparar a varios cañones por tick (round‑robin)
local function act_joetation(cli)
    ...
    logI("🎒 [joetation] pickup_holdable_from_pile")
    ...
    for n=1, math.max(1, CONFIG.joetation.triesPerAct) do
        ...
        logI(string.format("💥 [joetation] intento %d/%d → cannon=%d → ok=%s",
            n, CONFIG.joetation.triesPerAct, idx, tostring(ok)))
        okAny = okAny or ok
    end
    if not okAny then logW("💤 [joetation] sin disparo exitoso este tick, reintentando…") end
    tuneCad("joetation", okAny)
end

-- Coconut Bonk: recoger siempre y usar contra varios barcos por tick (round‑robin)
local function act_coconut(cli)
    if not canAct("coconut_bonk") then return end

    rpc(cli, "pickup_droppable", pileIdx)
    logI(string.format("🎒 [coconut] pickup_droppable → pile=%d", pileIdx))
    pileIdx = (pileIdx % CONFIG.coconut.pilesToCycle) + 1

    local uids = shipUidList(cli)
    if #uids == 0 then
        logW("🛶 [coconut] aún no hay ships_by_uid visibles; reintentamos…")
    ...
    for n=1, math.max(1, CONFIG.coconut.usesPerAct) do
        ...
        logI(string.format("🔧 [coconut] intento %d/%d → uid=%s → ok=%s",
            n, CONFIG.coconut.usesPerAct, tostring(uid), tostring(ok)))
        okAny = okAny or ok
    end
    if not okAny then logW("💤 [coconut] sin uso exitoso este tick, reintentando…") end
...
if CONFIG.enableAFK then
    LP.Idled:Connect(function()
        ...
        logI("🖱️ Anti‑AFK: click simulado.")
    end)
end

------------------------------ Bucle principal ------------------------------
task.spawn(function()
    logI("✨ Listo. AUTO =", CONFIG.autoEnabled and "ON" or "OFF")
    ...
    while true do
        if CONFIG.autoEnabled then
            ...
            if not anyPlaying then
                -- Elegimos por ETA y TP
                local best = pickBest()
                if currentPlaying ~= nil then logS("🏁 Partida anterior finalizada.") end
                currentPlaying = nil

                local target = CONFIG.coords[best] or CONFIG.coords.joetation
                logI(string.format("🧭 TP → %-13s @ (%.1f, %.1f, %.1f)", best, target.X, target.Y, target.Z))
                tpTo(target)
                ...
                if CONFIG.sendJoinMessageAfterTP then
                    ...
                    logI("📨 join_queue (opcional) enviado.")
                end

                task.wait(2)
            else
                -- En partida: jugar hasta el final
                if currentPlaying ~= playingId then
                    currentPlaying = playingId
                    cannonCursor, shipCursor, pileIdx = 1, 1, 1
                    logS(string.format("🎮 Entraste a %s. Autoplay FULL ON.", currentPlaying))
                end
