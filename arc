...
local function colorTable(color) -- sin hex
	return { __type="Color3", r=color.R, g=color.G, b=color.B }
end

local function colorTableHex(color)
...
-- Serializador genérico (sin ciclos complejos). Para streaming simplifico:
local MAX_DEPTH = 4
local function serializeValue(v, includeHex, depth, visited)
	...
	elseif t == "table" then
		visited = visited or {}
		if visited[v] then return {__ref=visited[v]} end
		visited[v] = "1" -- marcador simple (no guardamos id detallado aquí)
		local out = {}
...
-- Captura del modelo SOLO UNA VEZ -> luego se usa para básico y extendido
...
local function guiStreamAppend(sub, buffers, text)
	-- buffers: {current=string, index=int, chunkSize=int}
...
local function openFileStreaming(filename, header)
	...
	if useAppend then
		writefile(filename, header) -- crear nuevo
	else
		writefile(filename, header) -- usaremos concatenación final
	end
	return {
		filename = filename,
		useAppend = useAppend,
...
local function DumpHouse()
	...
	task.spawn(function()
		...
		-- Preparar lista de muebles (ordenados)
		...
		-- Encode cabecera sin el array (manualmente)
		local function encodeHeader(tbl)
			...
			-- json = {"time":..,"location":..,"house":..,"furniture_count":N}
			return json:sub(1,#json-1)..',"furniture":[' -- quitar cierre }
		end
		...
		for i,entry in ipairs(furnitureList) do
			...
			-- Serializar datos comunes
			local furnitureDataCommon = {
				id = data.id,
				cframe = serializeValue(data.cframe,false),
...
function App:render()
	...
	local FrameMain = Roact.createElement("Frame",{
		...
		DumpBtn = Roact.createElement("TextButton",{
			Text=DUMP_IN_PROGRESS and "Dump en curso..." or "Dump now",
			...
			AutoButtonColor= not DUMP_IN_PROGRESS,
